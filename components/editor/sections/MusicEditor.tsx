...
export default function MusicEditor({ initialData }: { initialData: ReadmeData }) {
  ...
  const [currentCategoryTab, setCurrentCategoryTab] = useState<'musicians' | 'albums' | 'songs'>('musicians');

  const { saveStatus: saveStatusMusic, errorMessage: errorMusic } = useSectionSave('music', music);
  const { saveStatus: saveStatusHiphop, errorMessage: errorHiphop } = useSectionSave('hiphop', hiphop);
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-3">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none">
          <button
            type="button"
            onClick={() => setCurrentCategoryTab('musicians')}
            className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all border ${
              currentCategoryTab === 'musicians'
                ? 'bg-teal-500/20 text-teal-600 dark:text-teal-400 border-teal-500/30'
                : 'bg-white/10 text-gray-600 dark:text-gray-400 border-transparent hover:bg-white/20'
            }`}
          >
            音乐人 / 厂牌 ({currentData.musicians?.length || 0})
          </button>
          <button
            type="button"
            onClick={() => setCurrentCategoryTab('albums')}
            className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all border ${
              currentCategoryTab === 'albums'
                ? 'bg-teal-500/20 text-teal-600 dark:text-teal-400 border-teal-500/30'
                : 'bg-white/10 text-gray-600 dark:text-gray-400 border-transparent hover:bg-white/20'
            }`}
          >
            最爱专辑 ({currentData.albums?.length || 0})
          </button>
          <button
            type="button"
            onClick={() => setCurrentCategoryTab('songs')}
            className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all border ${
              currentCategoryTab === 'songs'
                ? 'bg-teal-500/20 text-teal-600 dark:text-teal-400 border-teal-500/30'
                : 'bg-white/10 text-gray-600 dark:text-gray-400 border-transparent hover:bg-white/20'
            }`}
          >
            推荐单曲 ({currentData.songs?.length || 0})
          </button>
        </div>
      </div>

      {/* Conditional Editor rendering based on 2nd level tab */}
      <div className="pt-2">
        {currentCategoryTab === 'musicians' && (
          <ObjectArrayEditor
